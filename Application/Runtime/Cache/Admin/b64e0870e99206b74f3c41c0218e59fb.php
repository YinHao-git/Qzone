...
<html lang="en">
	...
	<center><h1>后台用户修改</h1>
	<form action="<?php echo U('Admin/update');?>" method="post" enctype="multipart/form-data">
	<table border=2 width="1000" cellspacing=0>
			<p><input type="hidden" name="id" value="<?php echo ($_GET['id']); ?>"></p>
			<p>姓名:<input type="text" name="name" value="<?php echo ($date['name']); ?>"></p>

			<p>密码：<input type="password" name="pwd" value="<?php echo ($date['pwd']); ?>"></p>
		
			<p>性别:<input type="radio" name="sex" value="0" <?php if($date["sex"] == '0'): ?>checked<?php endif; ?>>女
			<input type="radio" name="sex" value="1" <?php if($date["sex"] == '1'): ?>checked<?php endif; ?>>男</p>

			<p>年龄:<input type="text" name="age" value="<?php echo ($date['age']); ?>"></p>

			<p>地址:<input type="text" name="address" value="<?php echo ($date['address']); ?>"></p>

			<p>权限:<input type="radio" name="status" value="0" <?php if($date["status"] == '0'): ?>checked<?php endif; ?>>超级管理员
			<input type="radio" name="status" value="1"  <?php if($date["status"] == '1'): ?>checked<?php endif; ?>>普通管理员</p>
				
			<p>手机号：<input type="text" name="phone" value="<?php echo ($date['phone']); ?>"></p>
